...
// compileCFiles compiles C, C++, and assembly (.s/.S) files, returning object file paths.
// In cgo mode, all assembly files are compiled with the C compiler (not go tool asm).
func compileCFiles(cc, cxx, cgowork, srcDir, uid string, files gofiles.PkgFiles, cgoCflags, cgoCxxflags []string) ([]string, error) {
	...
	// Compile assembly files (.s/.S) with C compiler (cgo mode).
