...
func runCompilePackageCmd(args []string) {
	...
	// Write importcfg entry for consumers. When the interface split is on,
	// downstream compiles must read the export-data-only file so their
	// inputs don't change when only the link object does.
	if *importcfgOutput != "" {
		target := *output
		if *ifaceOutput != "" {
			target = *ifaceOutput
