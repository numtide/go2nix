# Parse go.mod in Nix.
# Returns a Nix attrset with the directive structure of the go.mod passed in.
#
# Derived from github.com/nix-community/gomod2nix@47d628dc (MIT).
# Changes: fix for go.mod files mixing single-line and parenthesized
# `require` blocks (upstream overwrites the attrset; we merge).
        ...
        in
        {
          data =
            ...
            // (
              ...
              # The default operation: single-line directive like "require foo v1.0".
              # If this directive already has an attrset (from a prior parenthesized
              # block), parse the value and merge instead of overwriting.
              # This handles go.mod files with mixed single-line and parenthesized
              # require blocks, e.g.:
              #   require ( ... )
              ...
              #   require ( ... )
                ...
                (
                  let
                    existing = acc.data.${directive} or null;
                    parsed =
                      let
                        m = match "([^ ]+) (.+)" rest;
                      in
                      if m != null then
                        ...
                        null;
                  in
                  if existing != null && typeOf existing == "set" && parsed != null then
                    {
                      ${directive} = existing // parsed;
                    }
                  else
                    {
                      ${directive} = rest;
